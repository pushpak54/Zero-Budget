...
// ─── Budget Storage ───────────────────────────────────────────────────────────
// Auth is handled by Supabase. Budget data stays in localStorage keyed to the
// Supabase user ID. Swap these two functions for Supabase DB calls later if needed.

const budgetStorage = {
  load: (userId) => {
    try {
      const v = localStorage.getItem(`budget_${userId}`);
      return v ? JSON.parse(v) : {};
    } catch { return {}; }
  },
  save: (userId, data) => {
    try { localStorage.setItem(`budget_${userId}`, JSON.stringify(data)); } catch {}
  },
};
...
const S = {
  ...
  authSuccess: { background: "#f0fdf4", borderLeft: "4px solid #22c55e", borderRadius: "0 10px 10px 0", padding: "14px 16px", marginBottom: 24, fontSize: 13, fontWeight: 500, color: "#166534" },
  ...
  btnAuth: { width: "100%", background: "linear-gradient(135deg,#2563eb,#4f46e5)", color: "#fff", padding: 16, borderRadius: 14, border: "none", fontWeight: 700, fontSize: 17, cursor: "pointer", boxShadow: "0 6px 20px rgba(79,70,229,0.4)" },
  btnAuthDisabled: { width: "100%", background: "#9ca3af", color: "#fff", padding: 16, borderRadius: 14, border: "none", fontWeight: 700, fontSize: 17, cursor: "not-allowed" },
  ...
  splash: { minHeight: "100vh", background: "linear-gradient(135deg,#0f172a 0%,#1e3a5f 50%,#1e1b4b 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif" },
...
const GLOBAL_CSS = `@keyframes spin{to{transform:rotate(360deg)}} @media(min-width:768px){.stat-resp{grid-template-columns:repeat(4,1fr)!important}}`;
...
function LoadingSplash() {
  return (
    <div style={S.splash}>
      ...
      <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 16, fontWeight: 500, marginTop: 24 }}>Loading Zero Budget…</p>
      <div style={{ marginTop: 24, width: 40, height: 40, border: "3px solid rgba(255,255,255,0.15)", borderTop: "3px solid #6366f1", borderRadius: "50%", animation: "spin 0.9s linear infinite" }} />
    </div>
  );
}
...
function InfoModal({ type, onClose }) {
  ...
  const handleSend = () => {
    if (!contactForm.name || !contactForm.email || !contactForm.message) { alert("Please fill in your name, email, and message."); return; }
    setSent(true);
  };
  return (
        ...
        <div style={S.infoBody}>
          {type === "tos" && (
            <div style={{ fontSize: 14, color: "#4b5563", lineHeight: 1.7 }}>
              ...
              <p style={{ marginBottom: 12 }}>By accessing or using Zero Budget ("the App"), you agree to be bound by these Terms of Service.</p>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: "#111827", margin: "20px 0 8px" }}>2. Description of Service</h3>
              <p style={{ marginBottom: 12 }}>Zero Budget is a personal budgeting tool that helps you track income, expenses, and financial goals.</p>
              ...
              <p>Authentication is handled by Supabase. Budget data is stored locally. The AI statement-parsing feature uses Anthropic's API.</p>
            </div>
          )}
          {type === "privacy" && (
            <div style={{ fontSize: 14, color: "#4b5563", lineHeight: 1.7 }}>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: "#111827", marginBottom: 8 }}>Our Commitment to Privacy</h3>
              <p style={{ marginBottom: 12 }}>Your financial data belongs to you and only you.</p>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: "#111827", margin: "20px 0 8px" }}>Authentication</h3>
              <p style={{ marginBottom: 12 }}>Sign-in is powered by <strong>Supabase Auth</strong>. Sessions persist across refreshes via secure JWTs. We never see your plaintext password.</p>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: "#111827", margin: "20px 0 8px" }}>Budget Data</h3>
              <p style={{ marginBottom: 12 }}>All budget entries are stored in your browser's localStorage keyed to your Supabase user ID. We do not transmit budget data to any server.</p>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: "#111827", margin: "20px 0 8px" }}>Third-Party Services</h3>
              <p>Statement parsing uses Anthropic's API. See <a href="https://supabase.com/privacy" target="_blank" rel="noreferrer" style={{ color: "#2563eb" }}>Supabase Privacy Policy</a> and <a href="https://www.anthropic.com/privacy" target="_blank" rel="noreferrer" style={{ color: "#2563eb" }}>Anthropic Privacy Policy</a>.</p>
            </div>
          )}
          {type === "contact" && !sent && (
            <div>
              <p style={{ color: "#4b5563", fontSize: 14, marginBottom: 20 }}>Have a question or found a bug? We'd love to hear from you.</p>
              {[["name","text","Your Name","Jane Smith"],["email","email","Email Address","jane@example.com"],["subject","text","Subject","e.g. Feature request…"]].map(([field,t,label,ph]) => (
                <div key={field} style={S.contactField}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 6 }}>{label}</label>
                  <input type={t} style={S.contactInput} placeholder={ph} value={contactForm[field]} onChange={(e) => setContactForm({ ...contactForm, [field]: e.target.value })} />
                </div>
              ))}
          ...
          {type === "contact" && sent && (
            <div style={{ textAlign: "center", padding: "24px 0" }}>
              ...
              <p style={{ fontSize: 14, color: "#4b5563" }}>Thanks <strong>{contactForm.name}</strong>! We'll reply to <strong>{contactForm.email}</strong> soon.</p>
            </div>
          )}
        </div>
...
function ProgressBar({ pct, style }) {
  ...
  return (
    <div style={{ height: 8, background: "#e5e7eb", borderRadius: 9999, overflow: "hidden", ...style }}>
...
function AuthScreen() {
  ...
  const switchMode = (m) => { setMode(m); setError(""); setMessage(""); setPassword(""); setConfirm(""); };
  ...
  const handleAuth = async () => {
    ...
    if (mode === "signup") {
      ...
      setLoading(false);
      if (err) {
        setError(err.message.includes("already registered") ? "An account with this email already exists. Please log in." : err.message);
      } else {
      ...
      if (err) {
        ...
        else if (err.message.includes("Email not confirmed")) setError("Please confirm your email address first.");
        else setError(err.message);
      }
      // On success → onAuthStateChange in root fires → user set → this screen unmounts
    }
  };
  ...
  const spinner = <span style={{ display: "inline-block", width: 18, height: 18, border: "2px solid rgba(255,255,255,0.4)", borderTop: "2px solid #fff", borderRadius: "50%", animation: "spin 0.8s linear infinite", verticalAlign: "middle", marginRight: 8 }} />;
    ...
    <div style={S.authScreen}>
      ...
      <div style={S.authWrap}>
        <div style={S.authBox}>
          ...
          {message && <div style={S.authSuccess}>{message}</div>}

          <div style={S.formGroup}>
            <label style={S.formLabel}>Email Address</label>
            <input type="email" style={S.formInput} placeholder="you@example.com" value={email}
              ...
              <div style={{ marginTop: 10 }}>
                ...
                {!pwdStrength.isValid && (
                  <div style={{ marginTop: 12, padding: 12, background: "#eff6ff", borderRadius: 10 }}>
                    ...
                    {[["length","At least 12 characters",pwdStrength.checks.length],["num","At least one number",pwdStrength.checks.hasNumber],["sym","At least one symbol (!@#$%...)",pwdStrength.checks.hasSymbol],["up","At least one uppercase letter",pwdStrength.checks.hasUpperCase],["lo","At least one lowercase letter",pwdStrength.checks.hasLowerCase]].map(([k,label,ok]) => (
          ...
          {mode === "signup" && (
            <div style={S.formGroup}>
              ...
              <input type="password" style={S.formInput} placeholder="••••••••••••" value={confirm}
                onChange={(e) => setConfirm(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAuth()} />
              {confirm && password !== confirm && <p style={{ fontSize: 12, color: "#dc2626", fontWeight: 500, marginTop: 6 }}>Passwords do not match</p>}
            </div>
          )}

          <button style={loading ? S.btnAuthDisabled : S.btnAuth} onClick={handleAuth} disabled={loading}>
            {loading ? <>{spinner}{mode === "login" ? "Signing in…" : "Creating account…"}</> : mode === "login" ? "Sign In" : "Create Account"}
          </button>

          {mode === "login" && (
            <button style={S.forgotBtn} onClick={handleForgotPassword} disabled={loading}>
              Forgot password?
            </button>
          )}
        ...
        <div style={S.authFooter}>
          ...
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>Secure budgeting platform powered by Supabase Auth</p>
        </div>
      </div>
...
function CategoryCard({ catKey, label, items, editingItem, isExpanded, onToggleExpand, onStartEdit, onSaveEdit, onDeleteItem, onAddItem }) {
  ...
  useEffect(() => { if (editRef.current) { editRef.current.focus(); editRef.current.select(); } }, [editingItem]);
  ...
  const handleAdd = () => { if (!newItemName.trim()) return; onAddItem(catKey, newItemName.trim()); setNewItemName(""); };
      ...
      {isExpanded && (
        <div style={{ padding: "0 24px 24px", borderTop: "1px solid #f3f4f6" }}>
          <div style={{ marginTop: 16 }}>
            ...
            {items.map((item) => {
              const isP = editingItem?.category === catKey && editingItem?.id === item.id && editingItem?.field === "planned";
              const isS = editingItem?.category === catKey && editingItem?.id === item.id && editingItem?.field === "spent";
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={S.itemGrid}>
                        <div>
                          ...
                          {isP ? <input ref={editRef} type="number" style={S.editInput} value={editVals.planned} onChange={(e) => setEditVals({ ...editVals, planned: Number(e.target.value) })} onKeyDown={handleKey} onBlur={saveEdit} />
                        ...
                        <div>
                          ...
                          {isS ? <input ref={editRef} type="number" style={S.editInput} value={editVals.spent} onChange={(e) => setEditVals({ ...editVals, spent: Number(e.target.value) })} onKeyDown={handleKey} onBlur={saveEdit} />
                      ...
                      {!isP && !isS && item.planned > 0 && <ProgressBar pct={iPct} style={{ height: 6, marginTop: 12 }} />}
                    </div>
                    <div style={{ marginLeft: 24, flexShrink: 0 }}>
                      <button style={S.btnIconRed} onClick={() => onDeleteItem(catKey, item.id)}>🗑</button>
                    </div>
                  </div>
                </div>
              );
          ...
          <div style={S.addRow}>
            <input type="text" style={S.addInput} placeholder={`Add item to ${label}`} value={newItemName}
              onChange={(e) => setNewItemName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAdd()} />
...
function UploadModal({ onClose, onSuccess }) {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      const cb = file.type === "application/pdf" ? { type: "document", source: { type: "base64", media_type: "application/pdf", data: base64 } } : { type: "image", source: { type: "base64", media_type: file.type, data: base64 } };
      const prompt = uploadType === "bill" ? 'Return ONLY a JSON array: [{"description":"Store","amount":9.99,"category":"food"}]. Categories: giving,savings,housing,transportation,food,personal,lifestyle,health,insurance,debt' : 'Return ONLY a JSON array: [{"description":"Store","amount":9.99,"date":"2024-01-15","category":"food"}]. Categories: giving,savings,housing,transportation,food,personal,lifestyle,health,insurance,debt';
      const resp = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 8000, messages: [{ role: "user", content: [cb, { type: "text", text: prompt }] }] }) });
      const data = await resp.json();
      const transactions = JSON.parse(data.content.map((i) => i.text || "").join("\n").replace(/```json|```/g, "").trim());
      if (!Array.isArray(transactions)) throw new Error();
      onSuccess(transactions); onClose();
    } catch { alert("Error processing file. Please ensure it is a valid statement."); }
    ...
    <div style={S.modalOverlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={{ ...S.modalBox, maxWidth: 480, marginTop: 40 }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <label style={{ ...S.formLabel, display: "block", marginBottom: 12 }}>Select Upload Type</label>
...
function PaycheckModal({ paychecks, totalPlanned, onAdd, onDelete, onClose }) {
  ...
  const handleAdd = () => { if (!form.date || !form.amount) return; onAdd({ id: Date.now(), date: form.date, amount: Number(form.amount), source: form.source || "Paycheck" }); setForm({ date: "", amount: "", source: "" }); };
  const getCashFlow = () => {
    const sorted = [...paychecks].sort((a, b) => new Date(a.date) - new Date(b.date));
    if (!sorted.length) return [];
    const flow = []; let running = 0;
    sorted.forEach((pc, i) => { running += pc.amount; const exp = i === sorted.length - 1 ? totalPlanned - flow.reduce((s, f) => s + f.expenses, 0) : totalPlanned / sorted.length; running -= exp; flow.push({ id: pc.id, date: pc.date, source: pc.source, income: pc.amount, expenses: exp, balance: running }); });
    return flow;
  };
  return (
    <div style={S.modalOverlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {[...paychecks].sort((a, b) => new Date(a.date) - new Date(b.date)).map((pc) => (
            <div key={pc.id} style={S.paycheckItem}>
              <div><div style={{ fontWeight: 600, color: "#111827" }}>{pc.source}</div><div style={{ fontSize: 13, color: "#4b5563" }}>{new Date(pc.date + "T12:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</div></div>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}><span style={{ fontWeight: 700, color: "#16a34a" }}>{fmt(pc.amount)}</span><button style={S.btnIconRed} onClick={() => onDelete(pc.id)}>🗑</button></div>
            </div>
          ))}
        </div>
        {paychecks.length > 0 && (
              ...
              {getCashFlow().map((f) => (
                <div key={f.id} style={S.cashFlowItem}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                    <div><div style={{ fontWeight: 600, color: "#111827" }}>{f.source}</div><div style={{ fontSize: 12, color: "#4b5563" }}>{new Date(f.date + "T12:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" })}</div></div>
                    <div style={{ textAlign: "right" }}><div style={{ fontSize: 13, color: "#4b5563" }}>Balance After</div><div style={{ fontSize: 20, fontWeight: 700, color: f.balance >= 0 ? "#16a34a" : "#dc2626" }}>{fmt(f.balance)}</div></div>
                  </div>
...
function FlowModal({ currentData, totalIncome, totalSpent, monthLabel, onClose }) {
  ...
  const breakdown = Object.entries(CATEGORY_LABELS).map(([key, label]) => { const items = currentData.categories[key] || []; return { category: label, spent: items.reduce((s, i) => s + i.spent, 0), planned: items.reduce((s, i) => s + i.planned, 0) }; }).filter((c) => c.spent > 0 || c.planned > 0).sort((a, b) => b.spent - a.spent);
    ...
    <div style={S.modalOverlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={{ ...S.modalBox, maxWidth: 900, marginTop: 40, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ background: "linear-gradient(135deg,#3b82f6,#4f46e5)", borderRadius: 16, padding: 24, color: "#fff", marginBottom: 24 }}>
          <div style={S.flowSummaryGrid}>{[["Total Income", totalIncome], ["Total Spent", totalSpent], ["Remaining", balance]].map(([l, v]) => (<div key={l}><div style={{ fontSize: 13, opacity: 0.9, marginBottom: 4 }}>{l}</div><div style={{ fontSize: 30, fontWeight: 700 }}>{fmt(v)}</div></div>))}</div>
        </div>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          {currentData.incomes.map((inc) => { const pct = totalIncome > 0 ? (inc.amount / totalIncome) * 100 : 0; return (<div key={inc.id} style={{ background: "#dcfce7", borderRadius: 12, padding: 16, border: "2px solid #bbf7d0", marginBottom: 12 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ fontWeight: 600, color: "#111827" }}>{inc.name}</span><span style={{ fontSize: 20, fontWeight: 700, color: "#16a34a" }}>{fmt(inc.amount)}</span></div><div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ flex: 1, height: 12, background: "#e5e7eb", borderRadius: 9999, overflow: "hidden" }}><div style={{ height: "100%", width: `${pct}%`, background: "#22c55e", borderRadius: 9999 }} /></div><span style={{ fontSize: 13, fontWeight: 600, color: "#15803d" }}>{pct.toFixed(0)}%</span></div></div>); })}
        </div>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          {breakdown.map((cat) => { const pct = totalSpent > 0 ? (cat.spent / totalSpent) * 100 : 0; const over = cat.spent > cat.planned; return (<div key={cat.category} style={{ borderRadius: 12, padding: 16, border: `2px solid ${over ? "#fca5a5" : "#fed7aa"}`, background: over ? "#fef2f2" : "#fff7ed", marginBottom: 12 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><div><div style={{ fontWeight: 600, color: "#111827" }}>{cat.category}</div><div style={{ fontSize: 13, color: "#4b5563" }}>Planned: {fmt(cat.planned)}{over && <span style={{ color: "#dc2626", fontWeight: 600, marginLeft: 8 }}>Over by {fmt(cat.spent - cat.planned)}</span>}</div></div><span style={{ fontSize: 20, fontWeight: 700, color: "#ea580c" }}>{fmt(cat.spent)}</span></div><div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ flex: 1, height: 12, background: "#e5e7eb", borderRadius: 9999, overflow: "hidden" }}><div style={{ height: "100%", width: `${pct}%`, background: over ? "#ef4444" : "#f97316", borderRadius: 9999 }} /></div><span style={{ fontSize: 13, fontWeight: 600, color: "#c2410c" }}>{pct.toFixed(0)}%</span></div></div>); })}
        </div>
        ...
        <div style={{ background: "linear-gradient(135deg,#f8fafc,#f1f5f9)", borderRadius: 16, padding: 28, border: "2px solid #e5e7eb", marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            {[{ label:"Income", value:totalIncome, sub:"Money In", color:"#22c55e" }, null, { label:"Expenses", value:totalSpent, sub:"Money Out", color:"#f97316" }, null, { label:"Balance", value:balance, sub:balance>=0?"Surplus":"Deficit", color:balColor }].map((item, idx) =>
              item === null ? (<div key={idx} style={{ flex: 1, minWidth: 48, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}><div style={{ width: "100%", height: 8, borderRadius: 9999, background: idx === 1 ? "linear-gradient(to right,#22c55e,#f97316)" : "linear-gradient(to right,#f97316,#3b82f6)" }} /><span style={{ fontSize: 11, color: "#6b7280", fontWeight: 600 }}>{idx === 1 ? "Flow →" : "→ Result"}</span></div>)
              : (<div key={item.label} style={{ textAlign: "center", flex: 1 }}><div style={{ borderRadius: 16, padding: "20px 28px", background: item.color, color: "#fff", boxShadow: "0 4px 12px rgba(0,0,0,0.15)", display: "inline-block", marginBottom: 10 }}><div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>{item.label}</div><div style={{ fontSize: 28, fontWeight: 700 }}>{fmt(item.value)}</div></div><div style={{ fontSize: 13, color: "#4b5563" }}>{item.sub}</div></div>)
            )}
          </div>
        </div>
...
function AppScreen({ user, onLogout }) {
  const [monthlyData, setMonthlyData] = useState(() => budgetStorage.load(user.id));
  ...
  useEffect(() => { if (Object.keys(monthlyData).length > 0) budgetStorage.save(user.id, monthlyData); }, [monthlyData, user.id]);

  useEffect(() => {
    const cd = getCurrentData();
    const isEmpty = cd.incomes.length === 0 && Object.values(cd.categories).every((a) => a.length === 0);
    if (isEmpty) setMonthlyData((d) => ({ ...d, [mKey]: initializeMonth(d[prevKey]) }));
  }, [mKey]);
  ...
  const addIncome = () => { if (!newIncome.name.trim() || !newIncome.amount) return; updateMonth({ incomes: [...cd.incomes, { id: Date.now(), name: newIncome.name, amount: Number(newIncome.amount) }] }); setNewIncome({ name: "", amount: "" }); };
  ...
  const addItem = (cat, name) => { const cats = { ...cd.categories }; cats[cat] = [...cats[cat], { id: Date.now(), name, planned: 0, spent: 0, isDefault: false }]; updateMonth({ categories: cats }); };
  const deleteItem = (cat, id) => { const cats = { ...cd.categories }; cats[cat] = cats[cat].filter((i) => i.id !== id); updateMonth({ categories: cats }); };
  const startEdit = (cat, item, field) => setEditingItem({ category: cat, id: item.id, field });
  const saveEdit = (cat, id, vals) => { if (!cat || !id) { setEditingItem(null); return; } const cats = { ...cd.categories }; cats[cat] = cats[cat].map((i) => i.id === id ? { ...i, ...vals } : i); updateMonth({ categories: cats }); setEditingItem(null); };
  const toggleCategory = (key) => setExpandedCategories((prev) => { const s = new Set(prev); s.has(key) ? s.delete(key) : s.add(key); return s; });
  const copyPrev = () => { const prev = monthlyData[prevKey]; if (prev) setMonthlyData((d) => ({ ...d, [mKey]: initializeMonth(prev) })); };
  const addPaycheck = (pc) => updateMonth({ paychecks: [...(cd.paychecks || []), pc].sort((a, b) => new Date(a.date) - new Date(b.date)) });
  const deletePaycheck = (id) => updateMonth({ paychecks: (cd.paychecks || []).filter((p) => p.id !== id) });
  const handleUploadSuccess = (transactions) => {
    const cats = { ...cd.categories }; let count = 0;
    transactions.forEach(({ description, amount, category }) => { if (description && amount && category && cats[category]) { const ex = cats[category].find((i) => i.name.toLowerCase() === description.toLowerCase()); if (ex) ex.spent += Number(amount); else cats[category].push({ id: Date.now() + count, name: description, planned: 0, spent: Number(amount), isDefault: false }); count++; } });
    updateMonth({ categories: cats }); showBanner(`Successfully added ${count} transactions!`, true);
  };
        ...
        <div style={S.headerCard}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div><h1 style={{ fontSize: 28, fontWeight: 700, color: "#111827" }}>Zero Budget</h1><p style={{ fontSize: 13, color: "#6b7280", marginTop: 2 }}>{user.email}</p></div>
            </div>
          ...
          <div style={{ marginBottom: 32 }}>
            ...
            {cd.incomes.map((inc) => (<div key={inc.id} style={S.incomeItem}><span style={{ fontWeight: 600, color: "#111827" }}>{inc.name}</span><div style={{ display: "flex", alignItems: "center", gap: 12 }}><input type="number" style={S.incomeInput} value={inc.amount} onChange={(e) => editIncomeAmount(inc.id, e.target.value)} /><button style={S.btnIconRed} onClick={() => deleteIncome(inc.id)}>🗑</button></div></div>))}
            <div style={S.addRow}>
            ...
            <div style={S.incomeTotalBox}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: 18, fontWeight: 600, color: "#374151" }}>Total Monthly Income</span><span style={S.incomeTotalAmt}>{fmt(totalIncome)}</span></div></div>
          </div>
          <div style={{ ...S.statGrid }} className="stat-resp">
            {[{ label:"Income", value:totalIncome, scheme:"blue" }, { label:"Budgeted", value:totalPlanned, scheme:"green" }, { label:"Spent", value:totalSpent, scheme:"orange" }, { label:"Remaining", value:remaining, scheme: remaining >= 0 ? "emerald" : "red" }].map(({ label, value, scheme }) => {
              const st = S.statCard(scheme);
              return (<div key={label} style={st}><div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>{label}</div><div style={{ fontSize: 24, fontWeight: 700 }}>{fmt(value)}</div></div>);
            })}
          </div>
        </div>
        {Object.entries(CATEGORY_LABELS).map(([key, label]) => (
          <CategoryCard key={key} catKey={key} label={label} items={cd.categories[key] || []} editingItem={editingItem} isExpanded={expandedCategories.has(key)} onToggleExpand={toggleCategory} onStartEdit={startEdit} onSaveEdit={saveEdit} onDeleteItem={deleteItem} onAddItem={addItem} />
        ))}
      </div>
...
export default function ZeroBudget() {
  // Three states: 'loading' (checking session) | 'authenticated' | 'unauthenticated'
  const [authState, setAuthState] = useState("loading");
  const [user, setUser] = useState(null);

  useEffect(() => {
    // Step 1 — restore existing session immediately on mount (this is what makes
    // users stay logged in after a page refresh)
    ...
    //   SIGNED_IN  → after login or email confirmation
    //   SIGNED_OUT → after logout
    //   TOKEN_REFRESHED → Supabase silently extends the session (no action needed)
    //   USER_UPDATED → password reset, email change, etc.
  ...
  const handleLogout = async () => {
    ...
    // onAuthStateChange fires automatically → sets state to 'unauthenticated'
  };
